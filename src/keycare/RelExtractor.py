...
from .relators.LLMRelator import LLMRelator
...
class RelExtractor:
    def __init__(
        ...
        self.rel_extractor = self.initialize_relation_method(
            language,
            n,
            thr_transformers,
            thr_setfit,
    ...
    def initialize_relation_method(
        self,
        language,
        n,
        thr_transformers,
        ...
        elif "llm" == self.relation_method:
            rel_extractor = LLMRelator(
                batch_size=batch_size, model=model, temperature=temperature
            )
        else:
            raise ValueError(
                "No relation method called {}".format(self.relation_method)
            )
    ...
    def __call__(self, source, target):
        ...
        if isinstance(source, Keyword):
            ...
            if all(isinstance(element, str) for element in source):
                ...
                raise TypeError(
                    "Source contains elements other than strings or Keyword class objects"
                )
        else:
            raise TypeError(
                "Source must be a string, a Keyword class object, a list of strings or a list of Keyword class objects"
            )

        if isinstance(target, Keyword):
            target = [target]
            ...
            if all(isinstance(element, str) for element in target):
                ...
                raise TypeError(
                    "Target contains elements other than strings or Keyword class objects"
                )
        else:
            raise TypeError(
                "Target must be a string, a Keyword class object, a list of strings or a list of Keyword class objects"
            )
        ...
        if self.all_combinations:
            ...
            self.relations = [
                Relation(
                    source_rep[i], target_rep[i], relations[i], self.relation_method
                )
                for i in range(len(source_rep))
            ]
        else:
            if len(source) == len(target):
                relations = self.rel_extractor.compute_relation(source, target)
                self.relations = [
                    Relation(source[i], target[i], relations[i], self.relation_method)
                    for i in range(len(source))
                ]
            else:
                raise TypeError(
                    "Source and target must be the same length when all_combinations=False."
                )
